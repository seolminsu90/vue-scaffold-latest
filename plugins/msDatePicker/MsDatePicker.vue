    ...
    <div class="ms-calendar-component position-relative" @click.stop.prevent="showCalandar = true">
        ...
        <transition name="ms-slide-down">
            <div class="ms-calendar-wrap" v-if="showCalandar" @click.stop.prevent="onCalendarFocus">
                <MsCalendarHeader :year="year" :month="month" :time-view="!isCalendarView" @onMove="onMove" />
                <transition name="ms-slide-up">
                    <div class="ms-calendar" v-if="isCalendarView">
                        <div class="ms-calendar-day week" v-for="(week, idx) in WEEKS" :key="idx">{{ week.toUpperCase() }}</div>
                        <div
    ...
    if (props.range) {
        if (!model.value.startDate) model.value.startDate = day
            ...
            if (model.value.startDate && model.value.startDate.f === day.f) {
                if (model.value.endDate) model.value.startDate = { ...model.value.endDate }
                model.value.endDate = null
                return
            }
            if (model.value.endDate && model.value.endDate.f === day.f) {
                model.value.endDate = null
                return
            }
...
const init = () => {
    ...
    if (props.range) {
        const st = modelValue.value[0].split(' ')
        const ed = modelValue.value[1].split(' ')
